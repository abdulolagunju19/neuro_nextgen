...
```
The first line uses commented out code (starts with #). This is code that will not be run by the compiler (this is what is turning what the english we write on the screen on spyder into a language the computer understands). It is good to use comments so the people who read your code understand what is going on. 
...
Now, using df.shape, we know there are 1338 rows and 7 columns. df.columns.tolist() gives us the names of the columns as a list. df.info shows us a table that gives us more detail about the kind of data we have, and df.nunique() shows the number of unique values for each column. This helps tell us if a column has a categorical or numerical variable (eg. we can see smoker is probably categorical as it has 2 unique values, bmi is numerical as it has 548). 
...
```
Before you analyze your code, you need to make sure it is clean. You don't want any missing values or duplicate rows. This is very basic data pre-processing. With datasets, you often need to clean it up much more before you can use it (taking care of outliers, etc.).
...
We then use len(df) to check the difference of the length of the dataframe after we use df.drop_duplicates to remove duplicate rows. Using this, we see that we remove 1 duplicate row.
...
```
In the code above, you see how you can use sns.distplot. This will create a histogram with a line superimposed. sns.heatmap will show you a heatmap of your data. You can also add titles and other attributes to your figures. 
