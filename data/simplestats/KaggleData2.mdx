...
In the following code:    `sample1 =  df[(df[category]==prop1)][var]` 
...
In that case, the computer would run the function with:    `sample1 =  df[(df["smoker"]=="yes")]["charges"]`
...
In that case, the computer would run the function with:     `sample2 =  df[(df["smoker"]=="no")]["charges"]`)

When I write:    `t, p = ttest_ind(sample1,sample2,equal_var = False)`
